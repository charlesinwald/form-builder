...
export function FieldAnalytics({ formData, responses }: FieldAnalyticsProps) {
  ...
  const averageRating = ratingData.length > 0 ? ratingData.reduce((a, b) => a + b, 0) / ratingData.length : 0
  ...
  const serviceField = formData.fields.find((field) => field.type === "select" || field.type === "radio")
  const serviceData = serviceField
    ? serviceField.options?.map((option) => ({
        name: option,
        value: responses.filter((r) => r.responses[serviceField.id] === option).length,
      })) || []
    : []
  ...
  return (
    <div className="space-y-6">
      ...
      {ratingField && (
        <Card>
          <CardHeader>
            <CardTitle className="text-lg font-serif">Average Rating</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center mt-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <span
                      key={star}
                      className={`text-lg ${star <= averageRating ? "text-secondary" : "text-muted-foreground"}`}
                    >
                      ⭐
                    </span>
                  ))}
              ...
              <div className="text-sm text-muted-foreground">Based on {ratingData.length} responses</div>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      {serviceField && serviceData.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-lg font-serif">{serviceField.label} Distribution</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
